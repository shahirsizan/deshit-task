...
const ProjectCard = ({ project, idx }) => {
	...
	return (
		<ScrollFadeIn>
			<motion.div
				key={idx}
				className="fadeInAnimation flex flex-col gap-2 md:gap-3 p-4 lg:p-6 rounded-2xl bg-[#11111F]/60 
				hover:border-1 hover:border-white hover:border-l-gray-600/80 hover:border-b-gray-600/80 shadow-[3px_3px_5px_3px_rgba(0,0,0,0.3)]"
			...
			>
				...
				<button className="w-fit flex items-center text-[#B94A5B] text-sm md:text-md cursor-pointer">
					Click to Visit{" "}
					<IoIosArrowForward className="text-[#B94A5B]" />
				</button>
			</motion.div>
		</ScrollFadeIn>
	);
